...
public class AlgoBooks {
    public static void main(String[] args) {
        SwingUtilities.invokeLater(AlgoBooks::createAndShow);
    }
    ...
    private static JPanel createBookCard(Book book, final JFrame frame) {
        ...
        buy.addActionListener(e -> {
            if("Data Structures".equals(book.title)) {
                frame.dispose();
                DataStructuresBook.main(new String[]{});
            }
            else if("Algorithms Illuminated".equals(book.title)) {
                frame.dispose();
                AlgorithmsIlluminatedBook.main(new String[]{});
            }
            else if("Fundamrntals Of Probability".equals(book.title)) {
                frame.dispose();
                FundamrntalsOfProbabilityBook.main(new String[]{});
            }
            else if("Introducing Algorithms in C".equals(book.title)) {
                frame.dispose();
                IntroducingAlgorithmsInC.main(new String[]{});
            }
            else if("JavaScript Programming For Data Analysis".equals(book.title)) {
                frame.dispose();
                JavaScriptBook.main(new String[]{});
            }
            else if("Java For Beginners".equals(book.title)) {
                frame.dispose();
                JavaBook.main(new String[]{});
            }
            else {
                frame.dispose();
            }
        ...
        card.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                else if("Software Architecture".equals(book.title)) {
                    frame.dispose();
                    SoftwareArchitectureBook.main(new String[]{});
                }
                else if("Java For Beginners".equals(book.title)) {
                    frame.dispose();
                    JavaBook.main(new String[]{});
    ...
    private static ImageIcon loadIcon(String path) {
        java.net.URL url = AlgoBooks.class.getResource(path.startsWith("/") ? path : "/" + path);
